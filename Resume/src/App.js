...
const Header = ({ visitCount, likeCount, onLike }) => (
  <Box
    bg="brand.500"
    color="white"
    py="4"
  ...
  >
    ...
    <Box position="absolute" top="4" right="4" textAlign="right">
      <Text>방문 수: {visitCount}</Text>
      <Text>좋아요: {likeCount}</Text>
      <Button onClick={onLike} colorScheme="red" size="sm">
        좋아요
      </Button>
    </Box>
  </Box>
...
const About = () => (
  <VStack align="start" p={4} bg="gray.100" borderRadius="md">
    ...
    <Text>
      저는 ~대학교에서 컴퓨터 과학을 전공하고 있으며, 데브옵스 분야에 큰 관심을
      가지고 있습니다. 클라우드 인프라, 자동화, CI/CD에 대한 지식을 갖추고
      있으며, 이를 프로젝트에 적용하는 데 열정적입니다.
    </Text>
  </VStack>
);
...
const Projects = () => (
  <VStack align="start" p={4} bg="gray.100" borderRadius="md">
    ...
    <Text>
      다양한 학교 프로젝트에 참여했으며, 특히 AWS를 사용한 웹 애플리케이션
      배포와 GitHub Actions를 통한 CI/CD 파이프라인 구축에 관심을 가지고
      있습니다.
    </Text>
  </VStack>
);
...
const Contact = () => (
  <VStack align="start" p={4} bg="gray.100" borderRadius="md">
    ...
    <Link href="https://github.com//yourgithubid" isExternal color="blue.500">
      Github: yourid
    </Link>
    <Link
...
const LearningProgressChart = () => {
  const data = {
    labels: [
      "완료: GitActions 및 CodePipeline을 이용한 CI/CD",
